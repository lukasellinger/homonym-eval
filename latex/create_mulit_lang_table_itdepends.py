...
def generate_multi_lang_table_it_depends(data):
    doc = Document()
    with doc.create(Table(position='h!')) as table:
        ...
        with table.create(Tabular('l' + 'c' * 5, width=6, booktabs=True)) as tabular:
            table.append(NoEscape(r'\textbf{Prompt / Model} & \multicolumn{5}{c}{\textbf{Sense Aware}} \\'))
            ...
            for prompt, data in data.items():
                tabular.add_hline()
                if prompt == 'child':
                    tabular.append(NoEscape(r'\multicolumn{6}{l}{\textbf{Prompt: ELI5}} \\'))
                elif prompt == 'simple':
                    tabular.append(NoEscape(r'\multicolumn{6}{l}{\textbf{Prompt: Simple}} \\'))
                elif prompt == 'normal':
                    tabular.append(NoEscape(r'\multicolumn{6}{l}{\textbf{Prompt: Normal}} \\'))
    ...
    doc.generate_tex('multi-lang-table-itdepends')
