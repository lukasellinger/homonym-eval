...
def generate_hown_overview_table(data):
    doc = Document()
    with doc.create(Table(position='ht')) as table:
        ...
        table.add_caption(
            'Response completeness and readability by prompt and model. The table shows the Flesch-Kincaid grade level (lower = easier), alongside the percentage of responses classified as \\textit{Complete}, \\textit{Full} (responses that cover all meanings without asking for context), \\textit{Context} (responses that request additional context without providing full coverage), and \\textit{Both} (responses that do both: cover all meanings and ask for additional context), as well as the average percentage of covered coarse-grained definitions.')
        table.append(NoEscape(r'\label{tab:hown-overview}'))

    doc.generate_tex('hown_overview_table')